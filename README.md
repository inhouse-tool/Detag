...
で, そんなデカい `.html` ファイルから会話部分のテキストだけを抽出して,
...
*Google* AI Overviews は, ユーザーからの質問文は冒頭の 2行程度で以下省略するのがデフォルトです.
...
としてセーブし直しているだけです.<br>
ただし, ブラウザでセーブしたのが `.htm` だった場合は `.html` にセーブし直します.
<br><sup>(
Firefox は `.htm` がお好みのようですが,
セーブしてから拡張子を変えようとすると Windows&reg; がなんか面倒なこと言い出すので.
)<sup>

拡張子の前のファイル名はオリジナルと同じで,<br>
`保存した会話.html` からは `保存した会話.md` または `保存した会話.htm` が,<br>
`保存した会話.htm` からは `保存した会話.md` または `保存した会話.html` が,<br>
オリジナルが置いてあったのと同じフォルダーにセーブし直されます.
...
1. ブラウザで AI さんの web ページを開く.
1. AI さんとの会話で盛り上がる.
1. ブラウザでページをセーブ ( <kbd>Ctrl</kbd>+<kbd>S</kbd> ) する.
1. 本アプリを起動する.
1. `.md` か `.htm` かを選ぶ.
1. セーブしておいた `.html` ( `.htm` ) を本アプリに Drag & Drop する.
1. 本アプリを閉じる.
1. オリジナルの `.html` ( `.htm` ) を削除する. ( 特に未練がなければ )
1. `.html` ( `.htm` ) に[接続](https://learn.microsoft.com/ja-jp/windows/win32/shell/manage#connected-files)していた `_files` も消えている. ( 特に削除したわけでもないのに )

※ 上記 8. と 9. はお好みですが, やっておくと容量の節約になって OneDrive&reg; の課金を遠ざけられます.
<sub>( まだ言うか. )</sub>

* 複数のファイルをつかんでいっぺんに Drop すると, 全部のファイルを取り扱います.
* `.html` や `.htm` じゃないファイルを Drop してもなにもしません.
...
日進月歩の業界です.
仕様がすぐ変わるんです.
「先週こういう言い方していたのに、今週はこんな言い方に変わってる。」なんてなことが,
毎週のように起きるのです.
こちらの想定外の仕様変更が行われると,
本アプリの抽出機能も一部空振りに終わることになりかねません.
もしそんな事態に遭遇したら,
ご自身で抽出ロジックに手を入れるとか,
オリジナルの HTML ファイルを捨てずにとっておいて本アプリが改版されるのを待つとか,
穏便なご対応をお願いいたします.
