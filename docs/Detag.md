...
Web 上での AI との会話を `.html` にセーブしたものを見てみると,
その中身はすごいことになっています.
なんかタグだらけで人間が目視で読めたものではありません.

で,
これらのタグを剥ぎ取ればどんな会話を交わしたのかのテキストが抽出できる,
んじゃないかと考えたわけです.
まずは `<` と `>` の間のテキストを抜いたイメージを出力するテストプログラムをでっち上げてみました.
それが本アプリの名前 `Detag` の由来です. 

で, そのタグ抜きの結果ですが, 想像と違いました.
会話中に見た覚えのないテキストがわらわらと混入しているんです.
会話のテキストも入ってはいたので,
セーブしたファイルに会話のテキストが残っているのは間違いなさそうです.
しかし, このタグ抜きの結果では, ゴミが多くて実用に向きません,

で, 会話に出てきた質問のテキストやその回答のテキストを `.html` から検索してみると,
それらのテキストをどう修飾するかのルールが AI ごとにあるようなのです.
なので, それらのルールに基づいて質疑のテキストだけを引っ張り出すためにまとめたのが, 以下の抽出処理です.

> [!IMPORTANT]
> 以下の処理は全て 2026年8月現在の各 AI の出力の仕様に基づくものです.
何せ~~仕様の安定しない~~日進月歩に進む業界ですので,
言っているそばからどこかが外れているかもしれません.
以下の記述とソースの内容が異なる場合は, ソースの方が正しく以下の記述が追い付いていないものとご判断ください.
抽出した内容に誤りがある場合は, ソースが AI の仕様に追い付いていないものとご理解ください.
...
1. `<` を検索する.
1. `<` を起点に次の `>` を検索する.
1. 前回の `>` と今回の `<` の間が空いていたら, それはタグに挟まれたテキストなので出力に回す.
1. 今回の `<` と `>` の間のテキスト ( つまりタグ ) を, 下記タイプごとに処理する:
	* [AI 非依存の共通処理](#ai-非依存共通処理)
	* [Copilot 固有の処理](#copilot-固有処理)
	* [Gemini web app 固有の処理](#gemini-固有処理)
	* [Google AI Overviews 固有の処理](#google-固有処理)
	* [全 AI 共用の処理](#全-ai-共用処理)
1. 以上の手順でタグを総ナメ.
1. 入力ファイルが Copilot のものだったら, [Copilot 専用調整処理](#copilot-専用調整処理)を呼ぶ.
1. 入力ファイルが Google AI Overviews のものだったら, [Google 専用調整処理](#google-専用調整処理)を呼ぶ.

また, Copilot と Google AI Overviews の場合は,
1ターンの質疑に 1セットずつ付いてくる ( こともある )
[サイテーション](https://www.google.com/search?q=AI+サイテーション)
( 情報源紹介欄 ) を振り返るため,
それぞれの AI で「ここから新しい質疑の始まりだな」と判断できるところに差し掛かるたびに,
Copilot では [Copilot 専用調整処理](#copilot-専用調整処理)の,
Google では [Google 専用調整処理](#google-専用調整処理)の呼び出しを, それぞれ挟んでいます.
また, これら専用調整処理は, 入力ファイルの読み込みが全て終わったあとにも,
もう 1回呼び出されます.
最後の質疑を振り返るためです.
<br><sup>(
Gemini web app には, 質疑ごとに振り替えるべき紹介欄は付いていません.
厳密には付いてないこともないのですが, セーブされないのです.
)</sup>

## [AI 非依存共通処理](#readme)

AI の種別に依存することなく問答無用でやってしまって構わない処理を, 下記のように片づけておきます.

* 会話に無関係な下表のタグを剥ぎ取る.<br>
...
* `<chat-window>` に遭遇したら, そこから出力を開始する.<br>
`<chat-window>` というのは Gemini 固有のカスタムタグです.
あまりカスタムタグに依存すると仕様変更に弱くなりそうですが,
Gemini の場合は他に見分ける手段がなかったので,
やむを得ず会話が始まった目印として使っています.

* `<user-profile-picture>` に遭遇したら, そこで出力を終了する.<br>
`<user-profile-picture>` というのも Gemini 固有のカスタムタグです.
あまりカスタムタグに依存すると仕様変更に弱くなりそうですが,
やむを得ず会話が終わった目印として使っています.

* `<h1>` は
「Gemini との会話 (Conversation with Gemini)」といった表題を大きく表示するためだけのものなので,<br>
 `</h1>` までを丸々剥ぎ取るだけで無視します.

* `<h2>` や `<h6>` は
Gemini からの回答文の始まりを示すものなので,
	* 次の `<p>` まで進む.
	* `<hr>\r\n<h6>\r\nGemini said\r\n</h6>\r\n` を出力に挟む.

* `<h5>` は
「ユーザーからの質問文」を示すものなので<br>
`<h6>` までをまず抽出し, その中から `<p>` で囲まれたテキストを質問文の 1行ずつとして抽出.<br>
`<hr>\r\n<h5>\r\nYou said\r\n</h5>\r\n` を挟んだ上, 抽出した質問文のテキストを出力に追加.

* `<div>` に対しては以下を実施:
	* とりあえず `>` まで剥ぎ取る.
	* `<div>` のネスト数をカウント中の場合は `+1` する.
	* `<div>` の中に `role="heading"` を含む場合は:
		* `aria-level="2"` を含む場合は「ユーザーからの質問文」ということなので<br>
		質問文を抽出し, `<p>` を `\r\n` に置換.<br>
		`<hr>\r\n<h5>\r\nYou said\r\n</h5>\r\n` を挟んだ上, 整形した質問文のテキストを出力に追加.

* `<span>` に対しては `>` まで剥ぎ取るだけ.

> [!NOTE]
> 「ユーザーの質問文の始まりを示す」目印が 2パターン ( `<h5>` と `aria-level="2"` を含む `role="heading"` な `<div>` ) ありますが,
2026年8月初頭までが `<div>` で表し, 以降 `<h5>` に変更された模様です.
<br><sup>( よく変わるんですよ, このへんの仕様. )</sup>


## [Google 固有処理](#readme)

* `<h2>` に対しては `</h2>` までを丸々剥ぎ取った上で [Google 専用調整処理](#google-専用調整処理)を実施.<br>
そして `<hr>\r\n<h5>\r\nYou said\r\n</h5>\r\n` を挟む.<br>
Google さん的に `<h2>` を出してきた場合,
それはユーザーの質問文の始まりを示すので,
Copilot さんと同じになるよう目印のテキストを挟んでおきます.<br>
ここで `<h2>` から `</h2>` まで丸々剥ぎ取ってしまっては,
元の質問文が失われないかと疑問に思われるかもしれませんが,
`<h2>` で囲まれているテキストは　Google さんの都合で目印として挟んでいるものらしく,
ユーザーに見せるための本当のテキストはこのかなり先に `<span>` で囲まれて再登場します.
<br><sup>( 
こういうことをやっているからムダにデカいんですよ…….
)</sup>

* `<h3>` に対しては `</h3>` までを丸々剥ぎ取るだけ.
( なんか中途半端に修飾されたユーザーからの質問文が入っているが, すぐ次に同じテキストが入ってくるので不要なだけ. )
...
* `<div>` に対しては以下を実施:
	...
	* カウント中でなく `data-subtree="aimc"` の属性を含む場合は:<kbd>NEW</kbd><br>
		* Google さんの回答文の始まりを示すので,
		出力に `<h6>\r\nGemini said\r\n</h6>\r\n` を追加.
...
* `<span>` に対しては `>` まで剥ぎ取った上で以下を実施:<br>
	* `role="heading"` の属性を含む場合は:
		...
		* `<span>` の中に `aria-level="1"` を含んでいたら,<br>
		...
		[Google 専用調整処理](#google-専用調整処理)を実施してから,
		出力に `<h5>\r\nYou said\r\n</h5>\r\n` を追加.

	* `data-key="aimhl"` または `data-subtree="aimfl"` の属性を含む場合は:<kbd>OLD</kbd><br>
		* Google さんの回答文の始まりを示すので,
		出力に `<h6>\r\nGemini said\r\n</h6>\r\n` を追加.
...
Google さんご本人がおっしゃってました.

> [!NOTE]
> 上記のようなことを Google さんご本人がおっしゃっいたにも関わらず,
しばらくして見返してみたら仕様が変わっていました.
[この仕様変更に関する Google さんご自身の弁明はこちら](./Q&AinGoogle.md).


## [全 AI 共用処理](#readme)

* `<textarea>` に遭遇したら, そこで出力を終了する.<br>
オリジナルの出力を直接ブラウザーで眺めている際には隠れていて見えませんが,
最後にユーザーからの反応を記入するためのテキスト入力欄が設けてある場合があります.
この欄より先には質疑に関係するテキスト成分は出てこないことが判っているので,
遭遇したらそこで出力は閉じておくことにしています.
...
* その他のタグの場合は以下を実施:<br>
`<a>` に対しては参照先 URL とラベル ( もしあれば ) 以外のアレコレは削除.<br>
`<img>` に対してはタグ全体を剥ぎ取った上, `alt=` として添えられていたテキストに差し替え.<br>
その他に対してはタグ全体を剥ぎ取る.

## [Copilot 専用調整処理](#readme)
...
* これまでの出力の最後が `</a>\r\n` で終わっていたら,<br>
ひと質疑終わったところで挟んでくるサイテーション ( 情報源紹介欄 ) を以下の要領で整理:
...
## [Google 専用調整処理](#readme)

質疑 1ターンごとに 1回ずつまとめて行う調整です.<br>
なので Google さんとの会話においてユーザーの質問文が現れるたび,<br>
そしてオリジナルを最後まで読み切ったときにもう一度最後の質疑を振り返るために行います.

その処理内容は以下の通りです.

* これまでの出力が空でなければ ( 最初のターンでなければ ) 以下を実施:
	* 情報源紹介欄 ( サイテーション ) を整理 ( 下記参照 ).
	* 以下の要領でソース・チップを通常のリンクに差し替え:
		* `>\r\n</a>` つまり中身が空っぽの `<a … ></a>` を検索.<br>
		( 文中に挟んだソース・チップのリンクを Gemini さんはそう表現するので. )
		* 見つからなかったら抜ける.
		* 見つかった `<a … ></a>` を出力中から一旦抜き取る
		* 見出し文字列がちゃんと入った `<a …>`……`</a>` を生成する.
		* 生成した `<a …>`……`</a>` を差し込む.
	* 以下の要領でユーザーからの質問文を修飾:
		* まず `</h5` を検索. ない場合は以下は省略.
		* `</h5` を起点に `<h6>` を検索.
		* `<h6>` を `<h5>` に差し替え. ( さすがに小さすぎるので )
		* その直後に置かれているユーザーの質問文を整形して修飾.
		* `</h6>` を `</h5>` に差し替え. ( さすがに小さすぎたので )
	* 以下の要領でコードリストの下に置かれた注意書きのフォントを縮小:
		* `</code></pre>\r\n<sup>` を検索.
		* 見つからなかったら頭から, 見つかったらその次から以下を繰り返し:
		* `</code></pre>\r\n` を検索.
		* 見つからなかったら抜ける.
		* 改行まで進む.
		* 改行がなかったらそっとしておく.
...
Google さんの場合は [Google 専用調整処理](#google-専用調整処理) を呼び出します.<br>
最後の質疑を締めくくるためです.
...
多少遠回りであっても今後の AI の仕様変更に耐えられるようにする,
というのが実装上の方針でした.
...
> [!TIP]
> 「日進月歩」の例:<br>
> サイテーションからピックアップしたテキストは, 大きく見出しを, その下に小さく情報源を添えていました.
...
<tr>
<td align="center">2026年8月初頭のサイテーション例
<td align="center">2026年8月中旬のサイテーション例
<tr>
<td align="center">見出し, 概要, 情報源 の順で並んでいる.<br>
最初のテキストを見出しに,<br>最後のテキストを情報源に回していた.
<td align="center">情報源, 見出し, 概要 の順で並んでいる.<br>
最初のテキストを情報源に,<br>次のテキストを見出しに回すようにした.
</table>

【参考】「いつ変えたんだ?」と問い詰められて「数週間おきにデザインが変わっています。」と白状する
[Goofle AI Overviews の回答](./CitationsChanged.md).
<br><sup>(
そんなに頻繁に変えられちゃあ, こちらも追い付かないじゃないのよ.
)</sup>
